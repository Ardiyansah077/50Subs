...
* Jangan lupa Subrek channelku ARDI Gans YT
...
const { help, bahasa, donasi, limitend, limitcount, bottt } = require('./50Subs/help')
...
prefix = "#"
name = "PATRICKSTAR"
rdaftar = "Nomer Dan Nama Anda Sudah Terdaftar Di Database Patrick😇"
rmenu = "HAI BRO👏 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 10
memberLimit = 2
ban = []
premium = ["6285282996146@s.whatsapp.net","62*********@s.whatsapp.net"]  // Isi Nomer Lu yang 62******
...
async function starts() {
	...
	frhan.on('qr', () => {
		console.log(color('[','red'), color('!','yellow'), color(']','red'), color(' Scan Tu ngab [ArdiGansYT]', 'green'))
	})

	fs.existsSync('./50Subs.json') && frhan.loadAuthInfo('./50Subs.json')
	frhan.on('connecting', () => {
		start('2', 'Connecting...')
	})
	frhan.on('open', () => {
	...
	await frhan.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./50Subs.json', JSON.stringify(frhan.base64EncodedAuthInfo(), null, '\t'))
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const Patrick = ["6285282996146@s.whatsapp.net"] // ubah aja gapapa
...
//akhir kreator
			   ...
			   case 'igstalk':
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.publikG)
				 data = fs.readFileSync('./50Subs/quotes.js');
		    ...
		    case 'darkjokes':
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./50Subs/drak.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 PATRICK 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 PATRICK 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
...
case 'asupan':
				...
				if (!isPublic) return reply(mess.only.publikG)
				reply(mess.wait)
				 data = fs.readFileSync('./50Subs/asupan.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
